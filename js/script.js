...
jQuery(document).ready(function ($) {
  ...
  function updateConnectionStatus(status, message = "") {
    ...
    switch (status) {
      case "connecting":
        ...
        statusText.text(displayMessage).addClass("connecting");
        statusLight.addClass("connecting").css("background-color", "#f0ad4e");
        break;
      case "connected":
        displayMessage = message || "Conectado";
        statusText.text(displayMessage).addClass("connected");
        statusLight.addClass("connected").css("background-color", "#5cb85c");
        break;
      case "disconnected":
        displayMessage = message || "Sin conexión";
        statusText.text(displayMessage).addClass("disconnected");
        statusLight.addClass("disconnected").css("background-color", "#d9534f");
        break;
    }
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            $suggestions.html(
              '<div class="pdm-error">Error al cargar IPs disponibles</div>',
            );
          }
        },
        error: function () {
  ...
  function searchProducts(search, isScanner = false, isDirectSearch = false) {
    debug("Iniciando búsqueda:", { search, isScanner, isDirectSearch });
    spinner.show();
    suggestions.hide();
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success && response.data.length > 0) {
          if (isScanner || isDirectSearch) {
            debug("Producto encontrado:", response.data[0]);
          ...
          debug("No se encontraron productos:", {
            search,
            isScanner,
            isDirectSearch,
          });
          if (isScanner || isDirectSearch) {
            Swal.fire({
              ...
              text: "No se encontró ningún producto con el código: " + search,
              icon: "warning",
            ...
            suggestions
              .html(
                '<div class="pdm-suggestion-item">No se encontraron productos</div>',
              )
              .show();
          }
        }
  ...
  // Manejar Enter en el campo de búsqueda para búsqueda directa
  barcodeInput.on("keydown", function (e) {
    if (e.which === 13 && !isScanning) {
      e.preventDefault();
      const search = $(this).val().trim();
      if (search) {
        searchProducts(search, false, true);
      }
    }
  });

  // Manejar click en el botón de búsqueda directa
  $("#pdm-search-btn").on("click", function () {
    const search = barcodeInput.val().trim();
    if (search) {
      searchProducts(search, false, true);
    } else {
      Swal.fire({
        title: "Campo requerido",
        text: "Por favor ingrese un código o nombre de producto",
        icon: "info",
        confirmButtonText: "Aceptar",
        confirmButtonColor: "#0073aa",
      }).then(() => {
  ...
  function displayProduct(product) {
    ...
    $("#pdm-product-sku").text(product.sku).data("product-id", product.id);
  ...
  $(".pdm-save-all").on("click", function () {
    ...
    // Stop all active measurements before saving
    $(".stop-measure-btn:visible").each(function () {
      $(this).click();
    });
      ...
      success: function (response) {
        ...
        if (response.success) {
          Swal.fire({
            ...
            text: "Los datos del producto se han guardado correctamente",
            icon: "success",
            confirmButtonText: "Aceptar",
            confirmButtonColor: "#0073aa",
          });
          ...
          Swal.fire({
            title: "Error",
            text: "Error al guardar los datos: " + response.data,
            icon: "error",
            confirmButtonText: "Aceptar",
            confirmButtonColor: "#d33",
          });
  ...
  async function getWeight() {
    ...
    // Usar cache si está disponible y no ha expirado
    if (
      measurementCache.weight.value !== null &&
    ...
    try {
      ...
      const weight = Number(data.peso_kg || data.weight || data.value).toFixed(
        2,
      );
  ...
  async function getDimension() {
    ...
    // Usar cache si está disponible y no ha expirado
    if (
      measurementCache.dimension.value !== null &&
      now - measurementCache.dimension.timestamp <
        measurementCache.cacheDuration
    ) {
  ...
  async function startContinuousWeight($input, $measureBtn, $stopBtn) {
    try {
      ...
      if (measurementInterval) {
        setTimeout(
          () => startContinuousWeight($input, $measureBtn, $stopBtn),
          MEASUREMENT_DELAY,
        );
      }
    } catch (error) {
      ...
      Swal.fire({
        ...
        text: `Error al obtener la medición: ${error.message}`,
        icon: "error",
        confirmButtonText: "Aceptar",
        confirmButtonColor: "#d33",
      });
  ...
  async function startContinuousDimension($input, $measureBtn, $stopBtn) {
    try {
      ...
      if (measurementInterval) {
        ...
        setTimeout(
          () => startContinuousDimension($input, $measureBtn, $stopBtn),
          MEASUREMENT_DELAY,
        );
      }
    } catch (error) {
  ...
  $(document).on("keydown", function (e) {
    ...
    if (
      ...
      !$(e.target).closest(".pdm-dimensions-form").length
    )
      return;
    ...
    switch (e.key) {
      case " ": // Espacio
        ...
        const $currentMeasureBtn = $(
          `.measure-btn[data-type="${currentField}"]`,
        );
        const $currentStopBtn = $currentMeasureBtn
          .nextAll(".stop-measure-btn")
          .first();
